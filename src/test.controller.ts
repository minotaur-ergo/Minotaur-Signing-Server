...
export class TestController {
  ...
  async mnemToXpub(@Body() body: any) {
    const xpubs = body.mnemonics.map((mnemonic: string) =>
      this.utilService.mnemonicToXpub(mnemonic),
    )
    return {
      xpubs: xpubs,
    }
  ...
  async pubToAddress(@Body() body: any) {
    ...
    const address = this.utilService
      .pubToAddress(pub)
  ...
  async signMessage(@Body() body: any) {
    ...
    return {
      signature: this.utilService.bytesToBase64(
        this.utilService.signMessage(mnemonic, message),
      ),
    }
  }
  ...
  async verifyJson(@Body() body: any) {
    ...
    delete data['signature']
    const verified = this.encryptService.verify(
